...
func _test_hurt_interrupt_and_death(player: Player, guard: CastleGuard) -> void:
	...
	source.begin_attack(7002, 1)
	source.try_hit(guard.hurtbox)
	source.begin_attack(7003, 1)
	source.try_hit(guard.hurtbox)
	_expect(guard.health_component.current_health == 0, "Three normal hits did not defeat 3-Health Guard")
