...
func _test_air_dash_attack() -> void:
	...
	_expect(is_equal_approx(player.stamina_component.current_stamina, 75.0), "Dash Attack charged its source Dash twice")
	await _wait_until_action_finished(actions, 40)
	await _wait_physics_frames(2)
	...
	await _press_once(PlayerScript.DASH_ACTION)
	_expect(actions.is_air_dash_active(), "A stamina-backed Air Dash did not start after Dash Attack")
	_expect(is_equal_approx(player.stamina_component.current_stamina, 50.0), "Follow-up Air Dash did not spend one charge")
