...
func _run_tests() -> void:
	...
	await _test_ground_dash_attack_followup()
...
func _validate_configuration() -> void:
	...
	if frames != null:
		...
		_expect(frames.get_frame_count(&"air_dash_start") == 2, "air_dash_start is not two frames")
		_expect(frames.get_frame_count(&"air_dash_loop") == 3, "air_dash_loop is not three frames")
		_expect(frames.get_frame_count(&"air_dash_end") == 2, "air_dash_end is not two frames")
		_expect(frames.get_animation_loop(&"air_dash_loop"), "air_dash_loop is not looping")
...
func _test_air_dash_and_dash_attack_costs() -> void:
	...
	await _wait_until_air_dash(player.action_controller, 40)
	_expect(is_equal_approx(player.stamina_component.current_stamina, 50.0), "Post-Dash-Attack Air Dash did not charge once")
	await _wait_until_action_finished(player.action_controller, 40)
	await _wait_until_grounded(player, 180)
	_expect(is_equal_approx(player.stamina_component.current_stamina, 50.0), "Landing refilled stamina")
	_cleanup_world(world)
	await process_frame


func _test_ground_dash_attack_followup() -> void:
	var world: Node2D = _create_world(false)
	var player: Player = _spawn_player(world)
	player.stamina_component.stamina_regen_rate = 0.0
	await _wait_physics_frames(5)
	await _tap_action(PlayerScript.DASH_ACTION)
	await _tap_action(PlayerScript.ATTACK_ACTION)
	_expect(player.action_controller.is_dash_attack_active(), "Ground Dash did not enter Dash Attack")
	_expect(is_equal_approx(player.stamina_component.current_stamina, 75.0), "Ground Dash Attack double-charged")
	await _tap_action(PlayerScript.DASH_ACTION)
	for frame_index: int in range(40):
		await physics_frame
		if player.action_controller.is_ground_dash_active():
			break
	_expect(player.action_controller.is_ground_dash_active(), "Ground Dash Attack did not continue from actual grounded contact")
	_expect(is_equal_approx(player.stamina_component.current_stamina, 50.0), "Post-Dash-Attack Ground Dash did not charge once")
	_cleanup_world(world)
	await process_frame
...
func _wait_until_air_dash(actions: PlayerActionController, maximum_frames: int) -> void:
	for frame_index: int in range(maximum_frames):
		await physics_frame
		if actions.is_air_dash_active():
			return
	_failures.append("Dash Attack did not continue into Air Dash")
...
func _finish() -> void:
	if _failures.is_empty():
		print("CHAIN_DASH_STAMINA_TEST: PASS (edge chaining, four charges, shared Air/Ground pool, HUD, collision)")
		quit(0)
		return
