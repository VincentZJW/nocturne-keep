...
- Produced three original project-owned cues with the local fixed-seed Python/NumPy/SciPy/FFmpeg pipeline: 184.615375-second Phase 1 `The Weight of the Last Key` at 78 BPM, 166.153854-second Phase 2 `The Gaol Breaks Within` at 104 BPM, and a 9.230771-second non-looping transformation bridge. The formal masters are 48 kHz stereo OGG; Standard MIDI, score JSON and analysis JSON are retained beside the authoritative source generator.
- Shared identity: original descending D–C–B-flat–A–E-flat gaoler motif, slow complete P1 statements and fragmented/displaced P2 variants. Low strings, synthesized bass brass, timpani, iron-gate impacts, chains, water pressure and wordless soul texture keep Chapter IV distinct from Chapter II waltz and Chapter III liturgy. No external sample, commercial melody, online generation API or real religious text is used.
- Replaced the obsolete 12-second WAV masters and removed their import metadata. The existing P1/P2 Track Resources now point to the formal OGG masters; a single clearly named transition Track Resource is registered. Runtime loop metadata is bar-aligned at 0–184.615375 and 0–166.153854 seconds.
- Boss integration: expanded the saved transition from 1.25 to 9.230769 seconds, retimed its existing 8 animation frames, and emitted five named sync cues at 1.153846/2.307692/4.615385/6.923077/8.653846 seconds. The room controller now ducks dialogue by 6 dB, lowers P1 during the bridge, starts the transition stinger exactly once, guards the P2 handoff, fades death over 2.0 seconds and restarts retry on P1. The existing `CH4_BOSS_PHASE_02` Main debug spawn now enters actual Phase 2 instead of aliasing a Phase 1 encounter.
- Fixed the existing `skip_intro_for_qa()` cancellation contract exposed by the new real-time bridge test: an intro sequence token now prevents an abandoned dialogue coroutine from resetting the Boss to Combat in the middle of PhaseTransition.
- Exact Godot 4.7.1 import/parse completed after the new OGG assets were imported. `test_soul_gaoler_music_ch4_m5.gd` passed through formal MainBootstrap with one real 9.23-second bridge, all five animation cues, ten Boss lifecycles, twenty one-shot transition guard cycles and a direct Phase 2 reload. Existing Boss runtime and Chapter IV Q4 route tests are part of final regression. Concurrent 900-second endurance playback passed with four Phase 1 wraps and five Phase 2 wraps; both held one active player and ended with static-memory growth of -2,888 bytes.
- Audio-analysis evidence: P1 peak -3.10 dBFS/RMS -16.68 dBFS/boundary delta 0; P2 peak -3.10 dBFS/RMS -17.27 dBFS/boundary delta 0; transition peak -3.10 dBFS/RMS -22.57 dBFS/boundary delta 0. Machine evidence establishes structure, headroom, continuity and runtime behavior; only the user's listening pass can accept subjective weight, fatigue and SFX balance.
- Scope remained audio/Boss-presentation only. Player systems, enemy AI, Boss damage/timing values, Chapter IV routing and unrelated dirty Chapter I/III files were not staged or changed by this milestone.

## 2026-08-08 — CH4-M0 Soul Gaoler Ormund Boss-music audit

Status: CH4-M0 audit complete; production remains `FAIL` until CH4-M1–M5 replace the two short cues, add a synchronized transition bridge and pass listening/Main QA

- Workflow loaded: root `AGENTS.md`, README, technical architecture, Chapter Scene Workflow, Chapter Character Workflow, production checklist, Chapter QA Standard, Game Audio Engineer skill and Godot Gameplay Scripter skill. Approved stop point is CH4-M0 only: audit current assets/runtime and lock the production plan; do not generate, replace or integrate new music.
- Formal authority: `run/main_scene` is `res://scenes/bootstrap/main_bootstrap.tscn`; the saved Boss arena is `res://chapters/chapter_04_drowned_underkeep/scenes/rooms/ch4_14_core_of_drowned_gaol.tscn`, with Ormund at `Enemies/SoulGaolerOrmund` and music orchestration in `chapter_04_boss_room_controller.gd` through the persistent `res://scripts/audio/music_manager.gd` Autoload and `Music` bus.
- Current assets: Phase 1 `soul_gaoler_phase_01_submerged_chains.wav` is 12.000 s, 50 BPM, 4/4, 22,050 Hz mono PCM; Phase 2 `soul_gaoler_phase_02_broken_cage.wav` is 12.000 s, 68 BPM, 4/4, 22,050 Hz mono PCM. Both definitions claim a 0–12 s loop, but the WAV files contain no `smpl` loop chunk, their import metadata has `edit/loop_mode=0`, and MusicManager applies loop flags only to OGG streams.
- Runtime proof: the existing long-play harness was run for 14 seconds against each Chapter IV track and both failed after 12 seconds with `Active player count changed to 0`. The present transition is only a guarded 0.85-second crossfade after the 1.25-second visual transition completes; there is no transition track/stinger, named music sync event, dialogue Duck, dedicated death ending or true direct-Phase-2 debug behavior.
- Production plan: keep 4/4; author an original 78 BPM, 150–210 s Phase 1 and 104 BPM, 150–210 s Phase 2 with one shared 4–7-note descending gaoler motif, multiple sections/rhythmic beds/harmonic environments, and a 6–12 s non-looping chain/cage/water transition bridge. Reuse the Chapter III fixed-seed Python/NumPy/SciPy score renderer, Standard MIDI writer, 48 kHz stereo Vorbis export, endpoint taper, analysis JSON, typed registry, two-deck crossfade, dialogue Duck and long-play/Main QA—not its melody or Chapter III liturgical palette.
- Planned ownership for later approved stages: directly replace the two existing formal runtime masters at their current paths (changing them from WAV to OGG requires updating the two existing track Resources and removing the obsolete WAV paths), replace the existing Chapter IV generator rather than create parallel `_v2/_new/_final` masters, and update the existing Boss room controller, track registry/definitions, Chapter IV Boss tests, music system spec and existing QA documentation. A transition asset does not currently exist, so CH4-M3 must add one clearly named formal cue rather than pretend a file can be overwritten.
- Verification baseline: exact Godot 4.7.1 editor/import/parse exited 0; formal MainBootstrap ran 240 frames and entered the opening cinematic with no red error. The two intentional 14-second loop probes exited 1 and are recorded as the reproduced current defect, not a new regression. No new audio was generated or auditioned in CH4-M0, and no commit is created for this audit-only documentation update.
...
- Workflow loaded: root `AGENTS.md`, README, technical architecture, Chapter Scene Workflow, Chapter Character Workflow, production checklist, Chapter QA Standard, render-layer contract, Godot Gameplay Scripter skill and Computer-use skill.
- Approved scope: fix the reported Chapter IV formal-flow defect where some ordinary/elite enemies remain dormant, cannot attack and cannot be hit, preventing Encounter clear and the route to Soul Gaoler Ormund. Do not change enemy balance, art, Player movement feel, Boss logic or Chapters I–III.
- Natural-input reproduction: starting from the saved `CH4_AREA_05` Main route and driving only the real `move_right`/`jump`/`dash`/`attack` Input Map actions, Encounter 01 activated and cleared normally. The Player then stopped at world `x≈856` before Encounter 02's ActivationArea at `x=1120`; all three second-group enemies remained visible but `PROCESS_MODE_DISABLED` with AI inactive, and Sewer Maw correctly retained its dormant Hurtbox-off state.
- Proven root cause: the formal room's second 12px-thick stepping-stone collision spans `x=868–932` at player-body height and is two-way. The earlier narrow repair changed only `PlatformCollision_00`; `PlatformCollision_01–03` and the equivalent authored thin platforms in other Chapter IV rooms remain two-way side walls. Existing tests bypassed the defect by disabling Player physics, directly swapping rooms, directly activating Encounter groups and directly applying health damage.
- Planned owned files: existing Chapter IV formal room TSCNs containing `PlatformCollision_*`, the existing formal-room builder so regeneration preserves the contract, the existing Main encounter regression, this log entry and existing QA documentation only. Existing unrelated dirty Chapter I/III/shared/QA work remains preserved and excluded.
- Required verification: exact Godot 4.7.1 parse/import; structural one-way-platform assertion across every saved Chapter IV formal room; natural Input Map traversal through the reproduced Cistern boundary; real attack-driven Encounter clear; formal Main encounter/transition/combat/Boss-route regressions; configured MainBootstrap smoke; GUI/Output/Debugger inspection; scoped diff and one meaningful commit.
- Implementation: all 23 authored `PlatformCollision_*` shapes across the nine Chapter IV rooms that contain elevated/stepping platforms are now one-way. The formal-room builder sets the same property when regenerating rooms. Floor, gate blocker, body, Hitbox, Hurtbox, Player movement and AI/tuning values are unchanged.
- Regression gap closed: the existing Main Encounter regression now starts with one real-input Cistern pass before its repeated deterministic route matrix. It drives only the public Input Map actions, clears both groups through real Player attacks, proves the former `x≈856` blocker is crossed, records 26 second-group health-change events, verifies the gate unlocks, enters ExitEast and reaches `CH4_AREA_06` through a real parsed `interact` event. It also asserts every saved Chapter IV `PlatformCollision_*` is one-way on every one of 165 formal room loads.
- Exact Godot 4.7.1 results: import/parse PASS; natural Cistern combat and `CH4_AREA_05 → CH4_AREA_06` handoff PASS; formal Main regression PASS (`5` route passes, `165` room loads, `200` Encounter activations, `460` instances/movement checks, `80` deaths, `10` checkpoint registrations, `8` roles); enemy runtime PASS; combat stress PASS (`360` actions, `48` Enemy contacts, `32` Player contacts, `160` deaths); S5 transitions PASS (`32/40/92/92/92/184/22/8`); Q4/Boss flow PASS; Main integration PASS; configured default MainBootstrap smoke PASS.
- Computer/Godot GUI evidence: the visible `Nocturne Keep (DEBUG)` Main run showed the Player beyond the old blocker with Encounter 02 actors present and active. Runtime output showed Sewer Maw `AI=true/Hurtbox=true/target=true`, HP `82→54→26`, Mirefin Raider `116→88→60→32→4`, Bog Toad `142→114→72→44→16`, followed by `LIVE_DEMO_PASS | Cistern cleared and formal ExitEast entered CH4_AREA_06`. No red script/resource/runtime error occurred.
- Scope check: no enemy art, damage/health, AI decisions, Player movement values, Boss logic, Chapter I–III content, project settings or Encounter composition changed. Existing unrelated dirty work remains preserved and excluded. This repair is a meaningful formal-flow fix and receives one isolated commit; stop after user handoff.

## 2026-08-06 — CH4-F4 full formal-route combat regression

Status: complete — exact Godot 4.7.1 formal Main regression and isolated QA commit complete; stop point is CH4-F4

- Workflow loaded: root `AGENTS.md`, README, technical architecture, Chapter Scene Workflow, Chapter Character Workflow, production checklist, Chapter QA Standard, render-layer contract and the Godot Gameplay Scripter skill.
- Approved scope: execute the final Chapter IV ordinary/elite enemy regression through the saved `MainBootstrap` route: every formal combat room, repeated Encounter activation, leave/return, checkpoint reload, movement/state recovery, bilateral real Hitbox/Hurtbox damage, death/clear and Output/Debugger checks. No new enemy behavior, tuning, art, room composition, Player, Boss or Chapter I–III modification is authorized.
- Baseline authority: `run/main_scene` is `res://scenes/bootstrap/main_bootstrap.tscn`; the formal Chapter IV level owns 17 saved rooms, ten combat rooms (`CH4_AREA_01–05`, `07–11`), twenty Encounter groups, 46 ordinary/elite instances and eight roles. Existing F1–F3 tests prove activation rollback, movement/state recovery and physical contact, but F4 must close the repeated full-route/return/checkpoint evidence as one explicit gate.
- Planned task-owned files: existing Chapter IV MainBootstrap regression tests only where coverage is missing, one existing Chapter IV QA report or this log entry, and no production gameplay file unless a new formal-route failure reproduces a Chapter IV-local defect. Unrelated dirty Chapter I/III/shared/QA work remains preserved and excluded.
- Required verification: exact Godot 4.7.1 import; five or more formal room loads and leave/return cycles; ten or more real Encounter activations; all eight roles and all authored actions; repeated Enemy→Player and Player→Enemy physical damage/dedup checks; ten deaths per role; both saved checkpoints; complete forward/reverse route; existing Boss-flow isolation; configured default F5 authority smoke; no red errors; scoped diff; one meaningful F4 commit; stop.
- F4 Main gate result: five complete forward/reverse passes loaded 17 rooms 165 times, activated and cleared all 20 formal Encounter groups 200 times, exercised 460 formal saved enemy instances and 460 chase responses, produced ten deaths for each of eight roles (80 total), and registered both saved checkpoint anchors five times each. Persistent Player/HUD identity, one-room ownership, outgoing-room release, spawn and Camera bounds passed on every load.
- Existing physical gates remained green: S5 completed 32 animated transitions, 40 real ActivationArea overlaps, 92 movement/action/Enemy→Player contacts, 184 Player→Enemy contacts, 22 shallow-water checks and all eight roles. Combat stress completed 360 authored action cycles, 48 Enemy contacts, 32 Player contacts, 160 deaths and five Encounter reloads with damage deduplication and facing assertions.
- Route isolation remained green: Broken Chainway completed ten transitions and five checkpoint reloads; the Boss route completed ten checkpoint/gate/intro/retry/death/reward/memory cycles; Q4 flow and Main integration passed. Default MainBootstrap 180-frame smoke selected the formal opening cinematic and exited 0.
- Exact engine/import: `/Users/vincentz/Downloads/Godot.app/Contents/MacOS/Godot` reported `4.7.1.stable.official.a13da4feb`; editor import/parse exited 0 with only the normal early-quit scan warning. Final valid commands contained no red script, resource, Output or Debugger error.
- Scope check: F4 changes only the existing Chapter IV Main regression, this log entry and `chapter_04_f4_combat_regression_report.md`. Production gameplay, Player, Chapter IV Boss, Chapters I–III, art, balance, music, narrative, loot and Encounter composition were not modified. Existing unrelated dirty work remains preserved and excluded. Manual acceptance remains combat feel/readability through the existing `CH4_START` F5 debug route; no later stage was entered.

## 2026-08-06 — CH4-F3 combat contact and facing repair

Status: implementation and exact Godot 4.7.1 physical-contact/MainBootstrap regression complete; stop point is CH4-F3 only

- Workflow loaded: root `AGENTS.md`, README, technical architecture, Chapter Scene Workflow, Chapter Character Workflow, production checklist, Chapter QA Standard, render-layer contract and the Godot Gameplay Scripter skill.
- Approved scope: audit and repair only Chapter IV ordinary/elite enemy attack damage, Player-to-enemy hits, left/right Hitbox placement and formal MainBootstrap combat contact. Player code, shared combat components, Chapter IV Boss, Chapters I–III, art, balance and encounter composition remain out of scope.
- Read-only baseline: all eight formal Chapter IV enemy scenes use Enemy body layer 4, Enemy Hurtbox layer 16/mask 32, Enemy Hitbox layer 64/mask 8 and two facing-root child hitboxes. The formal Player uses Hurtbox layer 8/mask 320 and attack layer 32/mask 16. Factions and vertical baselines are compatible; `FacingRoot.scale.x` is the intended horizontal mirror authority.
- Regression gap: the existing combat stress test advances action phases and applies direct Health damage, but never proves a physical Enemy Hitbox → Player Hurtbox contact or Player Hitbox → Enemy Hurtbox contact. CH4-F3 will first add those real-overlap assertions for every role, both facings and one-hit-per-action semantics, then modify only a reproduced Chapter IV-local root cause.
- Planned task-owned files: the existing Chapter IV enemy controller and/or its eight formal saved scenes only if the failing contact test proves a local geometry/routing defect, the existing Chapter IV combat stress test, and this isolated log entry. Existing unrelated dirty work remains preserved and excluded.
- Planned verification: exact Godot 4.7.1 import/parse; eight-role bilateral real-contact test with damage/dedup/facing assertions; existing enemy runtime, encounter, formal-route, Main integration and Boss-flow regressions; configured MainBootstrap smoke; Output/Debugger scan; scoped diff; one clear F3 commit; stop before F4.
- Reproduced root cause: the shared Chapter IV enemy scenes use fixed primary and secondary rectangles whose forward edges end at 73 and 99 pixels, while three stationary authored actions begin from longer configured ranges: Bog Toad `mud_burst` at 190, Underkeep Executioner `executioner_cleave` at 88 and `chain_reaper` at 118. AI entered a valid attack but the real Area2D never overlapped the Player at that authored distance. Both facings failed symmetrically; movement/projectile attacks and the faction/layer contract were already correct.
- Implementation: each Chapter IV enemy instance now duplicates its two rectangular hitbox shapes at runtime, captures their authored defaults and extends only a stationary, non-projectile active action to its existing configured range. The near edge is preserved so the extension cannot reach behind the enemy; geometry is restored whenever the attack closes. Moving attacks retain their authored motion bridge, projectiles remain unchanged, and `FacingRoot.scale.x` remains the single left/right mirror authority.
- Physical regression coverage: every one of the eight formal roles executed all three actions in both facings for 48 real Enemy Hitbox -> Player Hurtbox contacts, exact configured damage and same-attack deduplication. Forty-two non-projectile cases also proved forward-only placement. Player normal and Dash attacks produced 32 real Player Hitbox -> Enemy Hurtbox contacts across all roles and both facings, with exact damage and one-hit-per-attack semantics. Existing stress coverage still completed 360 action cycles, 160 deaths and 5 Encounter reloads.
- Formal MainBootstrap coverage: the saved Chapter IV route completed all 32 transitions and exercised 92 formal enemy-damage contacts plus 184 Player-damage contacts against the actual Main room instances, alongside 92 movement states, 92 action states, 22 shallow-water placements and all eight roles. The final suite reported PASS without replacing `run/main_scene` (`res://scenes/bootstrap/main_bootstrap.tscn`).
- Exact verification: Godot 4.7.1 import/parse exited 0; enemy runtime, Encounter runtime/manifests, formal route, Main encounters, Main integration, Q4 Boss flow, expanded combat stress and expanded S5 transitions all printed PASS. Default F5-authority headless smoke (`--quit-after 180`) entered the formal opening cinematic and exited 0. Final runs contained no red script, resource, Output or Debugger error; only the normal editor early-quit scan warning occurred during import.
- Scope check: no Player/shared-combat, Boss, Chapter I–III, art, tuning value, encounter composition, movement feel, jump, gravity or project-setting change belongs to CH4-F3. Existing unrelated dirty work remains preserved and excluded from this commit. Manual F5 acceptance should verify the long-range Bog Toad and Executioner attacks in both directions plus Player normal/Dash contacts; CH4-F4 was not entered.
...
## 2026-07-29 — Fallen Gate Knight Gatewarden Greatsword revision preflight

Status: in progress — formal runtime chain and current weapon proportions audited; Boss-sword-only redraw pending

### Goal and scope

- Redesign only the `Fallen Gate Knight / 堕落守门骑士` greatsword as the `Gatewarden Greatsword / 守门誓剑`: a restrained Gothic guard sword whose full visible length targets roughly 84–88% of the Boss's authored height, with a readable tapered point, layered blade facets, formal crossguard, wrapped grip and oath pommel.
- Apply one consistent weapon construction to Phase 1 and Phase 2, including idle, walk, turn, all sword attacks, hurt, transition and death presentation. Preserve Boss health, shield, AI, attack cadence, hitboxes, movement, bridge bounds, reward and chapter flow.
- The supplied knight image is proportion reference only and the supplied pixel sword is structure/material reference only; the production design remains original and Ravenmourn-specific.

### Read-only runtime audit

- Work begins on `master` at `0a0b092`. F5 authority is `res://scenes/bootstrap/main_bootstrap.tscn`; Chapter I is `res://chapters/chapter_01_ravenmourn_outskirts/scenes/level/ravenmourn_outskirts.tscn`; the live Boss is `World/CastleEntranceArea/FallenGateKnight`, instanced from `res://chapters/chapter_01_ravenmourn_outskirts/scenes/boss/fallen_gate_knight.tscn`.
- `VisualRoot/AnimatedSprite2D` reads `resources/boss/fallen_gate_knight_sprite_frames.tres`, which currently points to 165 formal 96×96 PNG frames under `assets/boss/fallen_gate_knight/sprites/`. The deterministic source is `scripts/tools/generate_fallen_gate_knight_art_v3.gd` and all live frames share `_draw_greatsword()`.
- The current weapon uses only a 7px post-extension and defaults to an approximately 40–50px grip-to-tip span. At native Main scale its crossguard and pommel compress into the hand, while the visible blade reads closer to a short one-handed sword than a gatekeeper's heavy longsword. Some action endpoints also clamp against the 96px cell, reducing apparent length inconsistently.
- The live scene contains no separate weapon Sprite or mount override; the sword is painted into each formal animation frame. Therefore the correct fix is a scoped generator/pose revision followed by regeneration of the authoritative PNGs and SpriteFrames verification, not a scene scale or gameplay reach change.
- Pre-existing unstaged Boss config serialization, Chapter I level serialization, enemy Resources, Player/loot Resources, Ravenfang QA images and UID sidecars are user-owned. This task will not stage or overwrite them; the Main level already references the authoritative Boss scene and needs no resource override.

### Planned task-owned files and verification

- Update only the focused Boss-art generator, generated Boss concepts/runtime PNGs/preview, focused manifest/spec, QA capture/test assets and this log. Archive a visual pre-change reference without restoring legacy runtime paths.
- Rebuild the 165 formal frames and SpriteFrames, run exact Godot 4.7.1 import/parse, focused art integrity, existing Boss combat regression, saved Boss scene/Chapter I smoke and actual MainBootstrap Boss-checkpoint capture.
- QA evidence must include old/new comparison, Phase 1 idle with Player, slash/thrust/heavy action, Phase 2 two-handed stance and death placement. All evidence will be visually inspected; any clipped tip, line-like blade, shield occlusion or inconsistent short frame is a FAIL.

## 2026-07-29 — Fallen Gate Knight Gatewarden Greatsword revision completion

Status: complete — Boss-sword-only concept, formal frames, Main evidence and regression checks delivered

### Delivered scope

- Replaced the short, one-handed-looking weapon with the original `Gatewarden Greatsword / 守门誓剑`. The neutral presentation targets approximately 84–88% of Boss height and uses a sharpened two-stage taper, pale cutting edge, old-silver and dark-iron blade planes, central ridge, gate-arch crossguard, 16px wrapped grip and faceted oath-seal pommel.
- Enlarged only the formal body/weapon frame cell from 96×96 to 128×96. Boss body drawing moved +16px within the wider cell, exactly cancelling the texture-center delta and preserving the existing feet/world anchor. Shield overlays remain 96×96. No collision, attack range, hitbox, damage, timing, AI, bridge or chapter-flow value changed.
- Regenerated all 41 animation families / 165 formal frames so Phase 1 and Phase 2 idle, walk, turn, slash, thrust, overhead, combo, jump, shockwave, hurt, transition and death presentations share one consistent weapon construction. Phase 2 retains its separate two-handed silhouette and restrained cold-blue fissure treatment.
- Expanded the deterministic pose endpoints so horizontal attacks have deliberate weapon room instead of clipping against the old 96px cell. Added an automated alpha-edge guard and minimum-reach checks for idle, slash, thrust, heavy and death samples.
- Updated the focused concept board, animation preview, manifest, Boss/art/quality specifications, README and QA index. The previous runtime preview and user-supplied baseline screenshot are preserved only as reference evidence; no live SpriteFrames path points to them.

### Saved scene and Main integration

- F5 authority remains `res://scenes/bootstrap/main_bootstrap.tscn`. Chapter I remains `res://chapters/chapter_01_ravenmourn_outskirts/scenes/level/ravenmourn_outskirts.tscn`; the live Boss is `World/CastleEntranceArea/FallenGateKnight` from `scenes/boss/fallen_gate_knight.tscn`.
- `VisualRoot/AnimatedSprite2D` continues to use the authoritative `resources/boss/fallen_gate_knight_sprite_frames.tres`. The formal Main instance has no SpriteFrames or texture override, so the regenerated formal inputs are the resources seen in F5. The pre-existing dirty level serialization was intentionally not edited or staged.
- Actual graphical QA launched `MainBootstrap`, selected Chapter I and used runtime spawn `boss_checkpoint`. It captured Phase 1 idle with Player, slash, heavy, Phase 2 idle, thrust, combo, death and an old/new comparison, then reset the transient debug-run configuration.

### Exact commands and actual results

1. `Godot --headless --path . --script res://chapters/chapter_01_ravenmourn_outskirts/scripts/tools/generate_fallen_gate_knight_art_v3.gd` — PASS: `animations=41 frames=165`.
2. Exact Godot 4.7.1 headless editor import/parse — exit 0; formal 128×96 body/weapon PNGs imported without a missing-resource, parser or duplicate-UID error.
3. `build_fallen_gate_knight_sprite_frames_v3.gd` — PASS: `animations=41 added_frames=69`; the authoritative SpriteFrames retained all existing runtime animation names.
4. `test_fallen_gate_knight_art_v3.gd` — PASS: `animations=41 frames=165 concepts=7 shield_states=4 main=true`, including new sword reach/edge tests.
5. `test_chapter_01_enemy_art_rework.gd` — PASS: `roles=6 formal_frames=340 archived_frames=290 main_refs=6`.
6. `validate_first_level_boss_assets.gd` — PASS: `206 action frames + 4 shield overlays`.
7. `test_first_level_boss.gd` — PASS: shield routing, phases, Main room lock/reset and death/exit; existing turn timing remained `1.1333 s`.
8. `capture_fallen_gate_knight_greatsword_revision_qa.gd` — PASS on OpenGL/Metal: `route=MainBootstrap spawn=boss_checkpoint captures=8`; no red Output/Debugger error.
9. Visual review of every capture — PASS: no clipped point, line-like blade, shield/weapon blob, mismatched short animation or death-floor penetration observed. Tutorial/debug overlays are disabled in the new captures.
10. Independent `scenes/tests/boss_test_room.tscn` headless smoke — exit 0 with no script/resource/runtime error.
11. Formal F5-equivalent `Godot --path . --quit-after 240` — exit 0 and entered `res://scenes/cinematics/opening_cinematic.tscn`, confirming the normal Main route was not replaced.

### Evidence and manual acceptance

- QA index: `docs/qa/fallen_gate_knight_greatsword_revision/README.md`.
...
## 2026-08-14 — CH4-FINAL-1–5 Soul Gaoler Ormund final adjustment

Status: implementation, automated Main/F5 QA and Godot desktop handoff complete; subjective music/combat feel remains the user's acceptance gate.

- Loaded the repository guide, README, technical architecture, permanent chapter scene/character/checklist/QA/layer policies and the Godot Gameplay Scripter skill.
- Approved scope is limited to Ormund Phase 1/2/transition music, the saved Soul-Lock Twin Keys reward interaction chain, Ormund visual/collision scale plus HP/damage-taken multipliers, focused tests/QA, and the existing `CH4_BOSS` desktop handoff. Chapters I–III, Chapter IV ordinary enemies, Player behavior, prior weapons, reward damage 16/32 and the current Boss attack rhythm are excluded.
- Audit authority: `res://scenes/bootstrap/main_bootstrap.tscn`; saved Boss room `res://chapters/chapter_04_drowned_underkeep/scenes/rooms/ch4_14_core_of_drowned_gaol.tscn`; formal Boss scene `res://chapters/chapter_04_drowned_underkeep/scenes/bosses/soul_gaoler_ormund.tscn`.
- Confirmed root causes/targets from CH4-FINAL-0: current 78/104 BPM scores lack the requested 6/8 forward motion; reward control restoration races the room transition and can restore `LOCKED` after the room has restored `FULL`; current Boss art is only 60% of original display scale rather than the prompt's assumed 88–90%, so the safe visual target is a 5% increase to 0.63, not 94% of the original oversized implementation. Balance targets are 500 HP and P1/P2 damage-taken multipliers 0.87/0.80 while preserving attack cadence, Player Turn, turn speed, tracking and punish windows.
- Planned verification: exact Godot 4.7.1 import/parse, music metadata/identity test, true InputMap E-event reward tests (10 collections plus duplicate/reload/flag checks), Boss runtime/balance/replay tests, MainBootstrap Boss→death→reward→memory route, rendered QA evidence, `git diff --check`, one meaningful milestone commit, then Godot desktop handoff at `CH4_BOSS`.
- Rebuilt both project-owned scores in 6/8: Phase 1 is 102 BPM/150.588229 seconds and Phase 2 is 128 BPM/150 seconds; the 128 BPM transition is 9.375 seconds so it covers the saved 9.230769-second eight-frame phase change and all five typed impact cues. The nine-note Ormund line, low-string pulse, syncopated cello pressure and five-to-seven-layer cap keep the fourth-chapter prison identity while adopting only Chapter III's compound-meter production method.
- Fixed the real reward fault in `Chapter04RewardController`: the reward presentation had captured the room transition's temporary `LOCKED` profile and restored that stale value after the prompt appeared. It now waits for the transition lock to release before owning/restoring its own presentation lock. The focused reward test now sends the real `interact` InputMap event instead of using `collect_for_qa`; ten clean-process runs passed, repeated E input retained one unique item, auto-equip remained 16/32 and the memory flag/exit were unlocked.
