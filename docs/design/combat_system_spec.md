...
Version: 2.0 — airborne normal enemy and first two-phase Boss
...
Player normal-Attack repetition uses one 0.15–0.20-second chain-input window, a 0.06-second single-entry buffer and a mandatory 0.06-second `AttackRecovery` beat after the complete four-frame animation. A valid repeat receives a fresh attack id only after that beat. Early/repeated spam cannot restart frame one, cannot skip `attack_04`, and cannot extend the one-entry buffer. This remains one repeated basic thrust, not a combo tree; Dash Attack timing and damage are unchanged.
...
- **Fallen Gate Knight:** Body 18 and Shield 10 compose the same Health/Hurtbox/Hitbox/Shield authorities. Frontal Player weapon attacks route exclusively to Shield until permanent break without overflow; rear attacks route only to Body. The contact frame records the old facing through synchronous routing, and the visual/Hitbox facing commit is deferred together, preventing a same-frame rear hit from becoming a frontal block. Shield visuals progress intact 10–8, damaged 7–5, critical 4–1, broken 0. Turn response is 0.10 s reaction + 0.13 s authored turn (0.2333 s at 60 Hz) with a 12 px threshold and 0.12 s cooldown. Existing attack animation tells are 8–10% shorter and shared Recovery is 0.42 s instead of 0.48 s; damage, active-frame indices and skill set are unchanged. Attacks lock direction through active frames and only allow a new turn from Recovery. Phase 1 owns Bash/Slash/Heavy; Phase 2 owns Combo/Jump Smash/Charge/Shockwave. Boss death completes presentation before the room unlocks.
...
F5 runs `res://scenes/main/main.tscn`. Seven one-shot groups contain 18 normal enemies sized 2/3/2/2/2/3/4, followed by a separate resettable Boss room. Main's closable debug panel reports Gargoyle state/lock/stun/target/height and Boss phase/Body/Shield/state/active Hitbox/room state via typed contracts.
...
No critical hits, armor formula, attributes, status effects, Player combo tree, drops, elite, second Boss, summons, random spawning, or final encounter balance is delivered.
