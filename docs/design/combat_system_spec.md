...
- **Fallen Gate Knight:** Body 180 and Shield 100 compose the same Health/Hurtbox/Hitbox/Shield authorities. Frontal Player weapon attacks route exclusively to Shield until permanent break without overflow; rear attacks route only to Body. `ShieldComponent` snapshots attack id/type, source, Boss position/facing and timestamp synchronously at contact, so a later turn cannot retroactively change rear routing. Normal hits always deal their routed damage but only trigger a 0.32-second lightweight flash cooldown; they never transition Boss state, restart Hurt, pause Turn or cancel windup/active attacks. Dash hits use a 0.50-second feedback cooldown and may cause only a 0.12-second reaction from neutral Idle/Approach/Recovery; Turn, all attack states, ShieldBreak, PhaseTransition and Death remain uninterruptible. The superseded 0.80–1.00-second target is now 0.33 s reaction + 0.80 s authored turn (1.1333 s measured at 60 Hz), with a 12 px threshold, 0.14 s cooldown and facing commit at 80% of the animation. Turn owns no active damage volume. The old shared `100×42 @ (65,4)` melee Area is replaced by Shield Bash `14×30 @ (19,4)`, Slash `26×22 @ (16,0)` and Thrust `32×10 @ (20,-7)` Areas under `FacingRoot`. Phase-1 weights are Bash/Slash/Heavy `22/43/35%`; Bash requires ≤37 px, cannot repeat consecutively and has a 2.70-second cooldown. Its authored stages are `0.46/0.10/0.68` seconds and its active-end gap is 1.18 seconds. Other active-end gaps remain Slash 1.05, Heavy 1.20, complete Combo 1.05, Charge 1.12, Jump 1.16 and Shockwave 1.10 seconds. Recovery movement is capped at 50% speed. Damage, active frames, skills and phases are unchanged.
