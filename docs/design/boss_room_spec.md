...
6. Crossing the enabled trigger emits the existing `level_completed` logic signal and asks the composed `CastleEntranceTransition` to fade for 0.55 seconds, then loads the minimal text-free `res://scenes/transitions/ravenmourn_threshold.tscn`. This is a visual threshold placeholder, not a developed second level.
