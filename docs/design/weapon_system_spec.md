...
After the Hollow Duchess death dialogue and mirror reveal, `Chapter02To03TransitionController` reconstructs the permanent Crimson Masque pickup at the saved `Chapter02BossWeaponPickupAnchor`, never at the Boss's final transform. E uses the same `WeaponPickup` API to add the unique id, auto-equip it, update the icon/Tier/14/28 HUD and set `chapter_02_boss_weapon_collected`. Reload before collection recreates it; reload after collection does not. The Royal Chapel Passage gate checks the story flag and inventory is the equipment source of truth.
