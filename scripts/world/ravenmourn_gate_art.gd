...
const GATE_VISUAL_WIDTH: float = 88.0
const GATE_VISUAL_HEIGHT: float = 260.0
...
func _draw() -> void:
	...
	draw_rect(
		Rect2(-GATE_VISUAL_WIDTH * 0.5, -GATE_VISUAL_HEIGHT * 0.5, GATE_VISUAL_WIDTH, GATE_VISUAL_HEIGHT),
		oak_dark
	)
	for plank_index: int in range(5):
		var plank_x: float = -41.0 + float(plank_index) * 17.0
		draw_rect(Rect2(plank_x, -126.0, 14.0, 250.0), oak)
		draw_line(Vector2(plank_x + 2.0, -122.0), Vector2(plank_x + 2.0, 120.0), oak_edge, 2.0)
	for band_y: float in [-100.0, -20.0, 60.0, 112.0]:
		draw_rect(Rect2(-48.0, band_y, 96.0, 9.0), iron)
		draw_line(Vector2(-46.0, band_y + 1.0), Vector2(46.0, band_y + 1.0), iron_edge, 2.0)
	for bar_x: float in [-38.0, -19.0, 0.0, 19.0, 38.0]:
	...
	for rivet_y: float in [-96.0, -16.0, 64.0, 116.0]:
		for rivet_x: float in [-38.0, 38.0]:
			draw_rect(Rect2(rivet_x - 2.0, rivet_y, 4.0, 4.0), Color(0.46, 0.4, 0.29, 1.0))
	for chain_x: float in [-43.0, 43.0]:
		for chain_y: float in range(-116, 110, 18):
			draw_rect(Rect2(chain_x - 3.0, chain_y, 6.0, 10.0), iron_edge, false, 2.0)
	...
	draw_circle(Vector2(0.0, 22.0), 11.0, iron_edge, false, 3.0)
	draw_rect(Rect2(-2.0, 31.0, 4.0, 16.0), iron_edge)
