...
func _test_saved_roster(groups: Array[EncounterGroup], enemies: Array[EnemyCombatant]) -> void:
	var shared_gargoyle_frames: SpriteFrames = load(
		"res://shared/resources/enemies/gargoyle_sentinel_sprite_frames.tres"
	) as SpriteFrames
	...
	for enemy: EnemyCombatant in enemies:
		...
		var is_chapter_enemy: bool = enemy.scene_file_path.begins_with(
			"res://chapters/chapter_01_ravenmourn_outskirts/scenes/enemies/"
		)
		var is_shared_enemy: bool = enemy.scene_file_path.begins_with("res://shared/scenes/enemies/")
		_expect(is_chapter_enemy or is_shared_enemy, "%s is not an enemy PackedScene" % enemy.name)
