...
const ROLE_DATA: Dictionary[String, Dictionary] = {
	"hollow_retainer": {"scene": ROOT + "/scenes/enemies/hollow_retainer.tscn", "idle": "idle", "size": 128},
	"court_halberdier": {"scene": ROOT + "/scenes/enemies/court_halberdier.tscn", "idle": "idle", "size": 128},
	"mourning_armor": {"scene": ROOT + "/scenes/enemies/mourning_armor.tscn", "idle": "idle", "size": 128},
	"blood_candle_acolyte": {"scene": ROOT + "/scenes/enemies/blood_candle_acolyte.tscn", "idle": "idle", "size": 128},
	"hanging_stalker": {"scene": ROOT + "/scenes/enemies/hanging_stalker.tscn", "idle": "hang", "size": 128},
	"hollow_duchess": {"scene": ROOT + "/scenes/boss/hollow_duchess.tscn", "idle": "idle", "size": 192},
}
...
func _run() -> void:
	...
	_expect(main != null, "Main bootstrap loads with Chapter II formal assets")
	print("CH2 CHARACTER REPLICATION 95 GATE | %s" % ("PASS" if _failures == 0 else "FAIL %d" % _failures))
...
func _validate_role(role: String, data: Dictionary) -> void:
	...
	if texture != null:
		...
		_expect(used.size.x >= (34 if expected_size == 128 else 68) and used.size.y >= (45 if expected_size == 128 else 68), "%s retains a readable signature silhouette" % role)
