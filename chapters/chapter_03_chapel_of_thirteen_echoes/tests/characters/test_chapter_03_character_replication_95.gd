...
const ROLE_DATA: Dictionary[String, Dictionary] = {
	"bellchain_penitent": {"scene": ROOT + "/scenes/enemies/bellchain_penitent.tscn", "idle": "idle", "size": 128},
	"censer_executioner": {"scene": ROOT + "/scenes/enemies/censer_executioner.tscn", "idle": "idle", "size": 128},
	"silent_chorister": {"scene": ROOT + "/scenes/enemies/silent_chorister.tscn", "idle": "idle", "size": 128},
	"stained_glass_seraph": {"scene": ROOT + "/scenes/enemies/stained_glass_seraph.tscn", "idle": "idle", "size": 128},
	"confessional_wraith": {"scene": ROOT + "/scenes/enemies/confessional_wraith.tscn", "idle": "idle", "size": 128},
	"thirteenth_scribe": {"scene": ROOT + "/scenes/enemies/thirteenth_scribe.tscn", "idle": "idle", "size": 128},
	"ossuary_penitent": {"scene": ROOT + "/scenes/bosses/ossuary_penitent.tscn", "idle": "idle", "size": 128},
	"choir_husk": {"scene": ROOT + "/scenes/bosses/choir_husk.tscn", "idle": "idle", "size": 128},
	"thirteenth_pontiff_edran": {"scene": ROOT + "/scenes/bosses/thirteenth_pontiff_edran.tscn", "idle": "prayer_idle", "size": 192},
}
...
func _run() -> void:
	...
	_expect(main != null, "Main bootstrap loads with Chapter III formal assets")
	print("CH3 CHARACTER REPLICATION 95 GATE | %s" % ("PASS" if _failures == 0 else "FAIL %d" % _failures))
...
func _validate_role(role: String, data: Dictionary) -> void:
	...
	_expect(texture != null, "%s idle texture resolves" % role)
	if texture != null:
		...
		_expect(used.size.x >= (32 if expected_size == 128 else 68) and used.size.y >= (38 if expected_size == 128 else 68), "%s retains a readable signature silhouette" % role)
