...
func _run() -> void:
	...
	_check(main_source.contains("chapter_04_character_trial.tscn"),"formal CharacterTrial is instanced in Main")
	_check(main_source.contains("CH4_BOSS_PHASE_01"),"Boss F5 spawn exists")
	_check(main_source.contains("CH4_BOSS_PHASE_02"),"Boss Phase II F5 spawn exists")
	_check(level_script_source.contains("complete_debug_phase_transition"),"Phase II spawn crosses the real transition contract")
	_check(main_source.contains("SoulGaolerOrmundHUD"),"Main owns Boss HUD")
	_check(trial_source.contains("soul_gaoler_ormund.tscn"),"Main trial owns latest Soul Gaoler scene")
